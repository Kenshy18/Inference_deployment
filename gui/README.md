...
ドライブ文字へ割り当てたネットワークドライブ（例:`O:\...`）も利用できます。
GUIは各ジョブの開始前に使用ドライブを列挙し、WSLで未マウントなら
`drvfs`として`/mnt/o`へ安全にマウントした後、入力の読取権限と出力先の書込権限を
確認します。この確認はジョブごとに行うため、WSL停止後にマウントが消えても
再実行時に復旧します。`\\server\share\...`のようなUNCパスは直接指定せず、
Windows側でドライブ文字へ割り当ててから選択してください。
