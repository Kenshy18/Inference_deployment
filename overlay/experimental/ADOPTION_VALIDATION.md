...
4025件、顔box 17031件を使用した。全モードでmask、顔、ラベルを有効にし、
8 Mbpsを指定した。高速版だけは対応済みの音声stream copyも有効にした。

| モード | 経過時間 | 処理速度 | 実時間比 | 映像frames | 結果 |
|---|---:|---:|---:|---:|---|
| CPU通常 | 41.763秒 | 126.67 fps | 4.23倍 | 5290 | pass |
| NVENC通常 | 38.083秒 | 138.91 fps | 4.64倍 | 5290 | pass |
| 高速6分割 | 3.940秒 | 1342.79 fps | 44.80倍 | 5290 | pass |

高速版はCPU通常の約10.6倍、NVENC通常の約9.7倍だった。実動画の高速出力では
次を確認した。
...
- 6 workerが合計4025 masks、17031 facesを描画
...
| 高速6分割 vs C++ lossless | 96.4271 | 0.996338 | 43.661 | 高速encode品質は良好 |
| 高速6分割 vs 高速単体 | 95.8298 | 0.995951 | 42.319 | 分割encodeも良好 |
| 高速6分割 vs 通常CPU | 86.4808 | 0.983547 | 29.965 | 描画方式差を含むため非同一 |

C++ lossless基準の600 framesでは、境界VMAF 96.5372、非境界VMAF 96.4224で、
境界固有の回帰はなかった。高速版と通常CPU版の比較でも境界VMAF 87.1197、
非境界86.4778であり、低い値はframeずれや結合不良ではない。通常版との差は主に
...
実測JSONと動画は
`overlay/output/adoption_validation_20260727/`以下に保存した。`output/`はGit管理外
なので、恒久的な判定根拠は本書と自動検証器を使用する。
