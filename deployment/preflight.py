        ...
        postprocess_probe = run(
            [
                ...
                (
                    ...
                    "curve_args=p.parse_args(['--input-jsonl','input.jsonl',"
                    "'--output-dir','output','--mask-geometry','catmull_rom']);"
                    "curve_stages=[s.implementation for s in "
                    "run_pipeline._configured_pipeline(curve_args).stages];"
                    "assert 'production.curve_v1_cpu' in curve_stages,curve_stages;"
                    "assert 'production.polygon_v3_cpu' not in curve_stages,curve_stages;"
                    ...
                    "assert 'production.curve_v1_cpu' in registered,registered;"
                    ...
                    "print(json.dumps({'stages':stages,'curve_stages':curve_stages,"
                    "'curve_cuda_used':False,'retired_options':"
        ...
        native_probe = run(
            [
                ...
                (
                    ...
                    "import native_interval_metrics as n,numpy as np;"
                    "v=n.exact_metrics([[[0,0],[4,0],[4,4],[0,4]]],"
                    "[[[0,0],[4,0],[4,4],[0,4]]]);"
                    "g=[[[[0,0],[4,0],[4,4],[0,4]]],"
                    "[[[1,0],[5,0],[5,4],[1,4]]]];"
                    "e=n.ExactDoubleRasterEvaluator(g);"
                    "c=np.asarray([g[0][0],g[1][0]],dtype=float)[:,None];"
                    "q=np.asarray(e.edge_metrics_batch(c,np.asarray([[0,0,1,0]],"
                    "dtype=np.int32),0.0,0.0,1,False));"
                    "assert q.shape==(1,5) and q[0,3]==1 and q[0,4]==1,q;"
                    "print(json.dumps({'module':n.__file__,'iou':v['iou'],"
                    "'recall':v['recall'],'curve_lazy_topology_api':True}))"
                ),
            ],
            cwd=root,
        )
