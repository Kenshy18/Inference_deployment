"""Family-owned DINOv3 ROI rich-spatial classifier model."""
...
class RoiRichSpatialAttnNoExpandedFusionClassifier(nn.Module):
    """Fuse local ROI, mask ROI, box-head, and geometry features by attention."""
    ...
    def __init__(
        ...
        if self.input_dim != expected_input_dim:
            raise ValueError(
                "input_dim mismatch for "
                "RoiRichSpatialAttnNoExpandedFusionClassifier: "
        ...
        if num_heads <= 0 or token_dim % num_heads != 0:
            raise ValueError(
                "attn_num_heads must divide attn_token_dim: "
                f"heads={num_heads}, dim={token_dim}"
            )
        self.token_pool_type = str(token_pool_type).strip().lower()
        if self.token_pool_type not in {"mean", "attn"}:
            raise ValueError(
                "token_pool_type must be one of ['mean','attn'], "
                f"got {token_pool_type}"
            )
        ...
        self.token_pool: nn.Module | None = None
        ...
        fused_dim = token_dim + (META_DIM if self.use_meta else 0)
        ...
        **kwargs,
        ...
        if roi_feat.ndim == 2:
            if int(roi_feat.shape[1]) != self.input_dim:
                raise ValueError(
                    f"roi_feat dim mismatch: expected {self.input_dim}, "
                    f"got {int(roi_feat.shape[1])}"
                )
...
__all__ = ["RoiRichSpatialAttnNoExpandedFusionClassifier"]
