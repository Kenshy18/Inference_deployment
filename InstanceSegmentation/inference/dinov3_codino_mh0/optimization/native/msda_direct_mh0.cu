...
constexpr int Q = 19320, H = 8, C = 32, L = 3, P = 4, CP = C / 4;
...
__global__ void msda_trt_exact(
  ...
  if (logical >= element_count) return;
  int t = int(logical);
  const int lane = t % CP;
  t /= CP;
  const int head = t % H;
...
void msda_cuda_out(
  ...
  TORCH_CHECK(
      value.dim() == 4 && value.size(0) >= 1
      && value.size(1) == Q && value.size(2) == H && value.size(3) == C
      && shapes.sizes() == at::IntArrayRef({L, 2})
      && starts.sizes() == at::IntArrayRef({L})
      && locations.sizes() == at::IntArrayRef(
          {value.size(0), Q, H, L, P, 2})
      && weights.sizes() == at::IntArrayRef(
          {value.size(0), Q, H, L, P})
  ...
  const int64_t element_count = value.size(0) * int64_t(Q) * H * CP;
  const int threads = 256;
  const int blocks = int((element_count + threads - 1) / threads);
  msda_trt_exact<<<
