#!/usr/bin/env python3
"""Run the interval-5 Phase-2 initial-candidate search."""
...
DEFAULT_OUTPUT = ROOT / "output/phase2_initial_candidates_interval5_20260810"
DEFAULT_PROFILES = (
    "scale_best",
...
def run_cell(
    ...
    environment["MASK_PIPELINE_PHASE2_TARGET_INTERVAL"] = str(
    ...
    python_paths = [str(POSTPROCESS), str(ROOT / "overlay/src")]
    if (
        args.cuda_fast
        or args.cuda_lazy_exact
        or args.cuda_hint_exact
        or args.native_exact
    ):
    ...
    if args.cuda_fast or args.cuda_lazy_exact or args.cuda_hint_exact:
        # CUDA dependencies intentionally live outside the Production venv.
        # Put the isolated site on PYTHONPATH for both the coordinator and its
        # process-pool workers.  Importing it only inside a worker makes CuPy
        # exception objects impossible to unpickle in the parent and hides the
        # actual CUDA failure behind ModuleNotFoundError: cupy_backends.
        python_paths.insert(
            0,
            os.environ.get(
    ...
    if process.returncode != 0:
        raise RuntimeError(f"Phase 2 failed: profile={profile} label={label}; {root/'run.log'}")
...
def main() -> int:
    ...
    for profile in profiles:
        ...
        payload = {
            ...
            "pair_vote": bool(
                args.pair_vote
                or args.pair_vote_constrained
                or args.pair_vote_per_key
            ),
            "pair_vote_mode": (
                "per_key_iou_recall"
