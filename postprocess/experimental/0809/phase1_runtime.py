#!/usr/bin/env python3
"""Production-v22 raw-only penalty DP with a hard per-frame Recall floor.

This module is an isolated experimental runtime.  It loads the unchanged
...
def _load_production_runtime() -> ModuleType:
    module_name = "experimental_0809_phase1_production_source"
...
def _patch_embedded_optimizer(module: ModuleType) -> ModuleType:
    ...
    def native_cached_interval_cost(*args, **kwargs):
        ...
        frame_loss_total, recall_budget_total, frames_covered = (
        ...
        update = (
            1.0
            if distance > float(runtime_args.shape_update_threshold_ratio)
            else 0.0
        )
    ...
    def build_frame_candidates(*args, **kwargs):
        ...
        for frame_index, candidates in enumerate(candidates_by_frame):
            ...
            candidates[0].recall_budget = float(
                recall_budget_from_metrics(exact)
            )
    ...
    def interval_cost_from_vectors(*args, **kwargs):
        ...
        precomputed_exact_deficit = kwargs.pop(
            "precomputed_exact_deficit", None
        )
        ...
        profile["production_seconds"] += float(
            time.perf_counter() - production_started
        )
        if (native_interval_enabled or precomputed_info is not None) and native_verify_remaining > 0:
    ...
    def run_single_state_penalty_path(
        ...
        target_interval = max(
            1, int(round(1.0 / max(float(args.target_ratio), 1e-6)))
        )
        dynamic_max_gap = max(
            int(args.max_gap),
            int(
        ...
        def decode(penalty: float) -> tuple[list[int], float, float]:
            costs = module.np.full((node_count,), module.np.inf, dtype=module.np.float64)
            ...
            for end in range(1, node_count):
                for start in range(predecessor_starts[end], end):
                    ...
                    if (
                        candidate_cost < float(costs[end]) - 1e-12
                        or (
...
def _write_audit(
    ...
    audit["implementation_contract_satisfied"] = all(
        bool(audit[key])
        for key in (
            "pair_vote_disabled",
            "raw_state_only",
