#!/usr/bin/env python3
"""Run the Phase-1 raw-only hard-Recall Production penalty matrix."""
...
DEFAULT_SOURCE_ROOT = ROOT / "output/production_raw_only_0809_20260809"
DEFAULT_OUTPUT_ROOT = ROOT / "output/phase1_raw_hard_recall_no_pair_20260809_final_v6"
...
def _discover_inputs(source_root: Path) -> dict[str, Path]:
    ...
    for group in manifest["groups"]:
        ...
        polygon_stage = next(
            stage for stage in details["stages"] if stage["id"] == "polygon_optimization"
        )
...
def _command(source: Path, output: Path, interval: int, args: argparse.Namespace) -> list[str]:
    return [
        sys.executable,
        str(RUNTIME),
...
def _metrics(
    ...
    *,
    ...
    with sqlite3.connect(f"file:{pred_sqlite.resolve()}?mode=ro", uri=True) as connection:
        ...
        prediction_rows = int(connection.execute("SELECT COUNT(*) FROM masks").fetchone()[0])
    ...
    return {
        ...
        "feasible_recall_violations": sum(
            value + 1e-12 < float(audit["recall_floor"])
            for value in feasible_recalls
        ),
        "feasible_iou_mean": float(np.mean(feasible_ious))
        if feasible_ious
        else 1.0,
        "feasible_iou_min": min(feasible_ious, default=1.0),
        "feasible_iou_q01": _quantile(feasible_ious, 0.01)
        if feasible_ious
        else 1.0,
...
def _aggregate(rows: list[dict[str, object]], interval: int) -> dict[str, object]:
    ...
    return {
        ...
        "feasible_iou_mean": sum(
            float(row["feasible_iou_mean"])
            * int(row["feasible_observation_rows"])
            for row in selected
        )
...
def _write_report(root: Path, rows: list[dict[str, object]], aggregates: list[dict[str, object]]) -> None:
    columns = [
        "label",
        "requested_interval",
    ...
    with (root / "phase1_results.csv").open("w", encoding="utf-8", newline="") as handle:
    ...
    for row in rows:
        lines.append(
            ...
            "{feasible_recall_violations} | {infeasible_streams} | "
            "{feasible_area_ratio_max:.3f} |".format(
                **row
            )
        )
...
def main() -> int:
    ...
    payload = {
        ...
        "execution": {
            ...
            "maximum_concurrent_dp_workers": min(
                int(args.label_workers), len(labels)
            )
