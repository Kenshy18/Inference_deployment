#!/usr/bin/env python3
"""Sweep track-wise worst-case boundary placement."""
...
from humanlike_vertex_placement_20260812.trackwise import (  # noqa: E402
    trackwise_worst_rdp_sequence,
)
...
def main() -> int:
    ...
    parser.add_argument("--dense-counts", default="64,96,128")
    ...
    rows = []
    best = None
    for dense_count in [int(value) for value in args.dense_counts.split(",")]:
        for quantile in [float(value) for value in args.quantiles.split(",")]:
            for count in [int(value) for value in args.counts.split(",")]:
                started = time.perf_counter()
                sequence = trackwise_worst_rdp_sequence(
                    polygons,
                    count,
                    dense_vertices=dense_count,
                    temporal_quantile=quantile,
                )
