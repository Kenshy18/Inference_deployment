#!/usr/bin/env python3
"""Sweep the temporal vertex-placement DP on one SQLite track segment."""
...
from humanlike_vertex_placement_20260812.temporal_dp import (  # noqa: E402
    temporal_band_dp_sequence,
)
...
def main() -> int:
    ...
    parser.add_argument("--bands", default="4,8,12")
    parser.add_argument("--temporal-weights", default="0.02,0.10,0.50")
    ...
    for band in [int(value) for value in args.bands.split(",")]:
        for temporal_weight in [
            float(value) for value in args.temporal_weights.split(",")
        ]:
            started = time.perf_counter()
            sequence = temporal_band_dp_sequence(
                polygons,
                args.vertices,
                band=band,
