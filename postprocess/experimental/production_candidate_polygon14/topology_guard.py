...
def local_key_update_is_simple(
    ...
    if position > 0 and first_invalid_edge_frame(
        module,
        run,
        ...
        return False
    if position + 1 < len(chosen_frames) and first_invalid_edge_frame(
        module,
        run,
        int(chosen_frames[position]),
        trial_vector,
...
def repair_decoded_path(
    ...
    while cursor + 1 < len(frames):
        ...
        stats["dp_selected_edges_checked"] = int(
            stats.get("dp_selected_edges_checked", 0)
        ) + 1
