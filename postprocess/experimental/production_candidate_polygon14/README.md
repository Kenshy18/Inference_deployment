...
14頂点のトラック整合ポリゴン近似と、凍結した既存のキーフレーム最適化を組み合わせたProduction候補です。まだ既定のProduction処理には接続していません。

## 処理契約

1. 入力SQLiteから、トラッキング後の毎フレーム・各連結成分の輪郭を読む。
2. 各トラックの各成分スロットを独立に、固定14頂点の `persistent_line_fit_quality_guarded` で近似する。
3. この14頂点列だけをDPの候補形状へ渡す。Recallの参照である `run.gt_polygons` は変更しない。
...
- 頂点数: 1連結成分につき14。トラック区間内で成分スロット数と頂点IDを維持する。
