...
class ExactPairVoteEvaluator:
    ...
    def __init__(
        ...
        self.bindings = [self._binding(frame) for frame in range(len(run.frame_numbers))]
        ...
        self.stats["evaluator_builds"] = int(
            self.stats.get("evaluator_builds", 0)
        ) + 1
    ...
    def _binding(self, frame: int) -> _FrameBinding:
        ...
        left = max(0, right - 1)
        alpha = float(
            (frame - self.chosen[left])
            / max(self.chosen[right] - self.chosen[left], 1)
        )
    ...
    def _aggregate(rows: list[dict[str, float]]):
        ...
        return (
            ...
            float(total_intersection / total_gt_area)
            if total_gt_area > 0
            else 1.0,
        )
    ...
    def full_metrics_many(
        ...
        self.stats["native_full_batches"] = int(
            self.stats.get("native_full_batches", 0)
        ) + 1
    ...
    def local_metrics_many(
        ...
        if self.native_batch is not None and trial_vectors:
            ...
            self.stats["native_batches"] = int(
                self.stats.get("native_batches", 0)
            ) + 1
        ...
        if len(trial_vectors) <= 1 or self.worker_count <= 1:
            return [
                self.local_metrics(current, key_pos, trial)
        ...
        self.stats["parallel_batches"] = int(
            self.stats.get("parallel_batches", 0)
        ) + 1
