#!/usr/bin/env python3
"""Deterministic role-based polygon candidates for Production.
...
def _s30_r2_a105(run, frame_index: int, slot: int) -> np.ndarray:
    return _support_area_variant(run, frame_index, slot, radius=2, area_factor=1.05)


def _s30_r2_a125(run, frame_index: int, slot: int) -> np.ndarray:
    return _support_area_variant(run, frame_index, slot, radius=2, area_factor=1.25)


def _s30_r5_a105(run, frame_index: int, slot: int) -> np.ndarray:
    return _support_area_variant(run, frame_index, slot, radius=5, area_factor=1.05)


def _s30_r5_a125(run, frame_index: int, slot: int) -> np.ndarray:
    return _support_area_variant(run, frame_index, slot, radius=5, area_factor=1.25)


def _s30_r10_a105(run, frame_index: int, slot: int) -> np.ndarray:
    return _support_area_variant(run, frame_index, slot, radius=10, area_factor=1.05)


def _s30_r10_a125(run, frame_index: int, slot: int) -> np.ndarray:
    return _support_area_variant(run, frame_index, slot, radius=10, area_factor=1.25)
...
def _frame_size() -> tuple[float | None, float | None]:
    width = (
        float(os.environ["MASK_PIPELINE_PHASE2_FRAME_WIDTH"])
        if os.environ.get("MASK_PIPELINE_PHASE2_FRAME_WIDTH")
        else None
    )
    height = (
        float(os.environ["MASK_PIPELINE_PHASE2_FRAME_HEIGHT"])
        if os.environ.get("MASK_PIPELINE_PHASE2_FRAME_HEIGHT")
        else None
    )
    return width, height
...
def _a07(run, frame_index: int, slot: int) -> np.ndarray:
    return _orthogonal_order_stat(run, frame_index, slot, kth=None, union_raw=False)
...
def _a06_k4(run, frame_index: int, slot: int) -> np.ndarray:
    return _orthogonal_order_stat(run, frame_index, slot, kth=4, union_raw=True)
...
def _directional_support_envelope(
    ...
    *,
    ...
    candidate_mask = _cap_union_area(candidate_mask, raw, max_ratio=float(max_ratio))
    polygon = _mask_to_polygon(candidate_mask, origin, reference)
    return reference if polygon is None else polygon
...
def _trajectory_residual_sweep(
    ...
    *,
    ...
    radial_cap = 0.35 * math.sqrt(max(_polygon_area(reference), 1.0) / math.pi)
    lengths = np.linalg.norm(shifts, axis=1)
    scale = np.minimum(1.0, radial_cap / np.maximum(lengths, 1e-9))
    shifts = shifts * scale[:, None]
    swept = np.stack([reference + shift[None, :] for shift in shifts], axis=0)
...
def _ctr4_125(run, frame_index: int, slot: int) -> np.ndarray:
    return _trajectory_residual_sweep(run, frame_index, slot, radius=4, max_ratio=1.25)


def _ctr4_150(run, frame_index: int, slot: int) -> np.ndarray:
    return _trajectory_residual_sweep(run, frame_index, slot, radius=4, max_ratio=1.50)


def _ctr8_150(run, frame_index: int, slot: int) -> np.ndarray:
    return _trajectory_residual_sweep(run, frame_index, slot, radius=8, max_ratio=1.50)
...
def _vf6(run, frame_index: int, slot: int) -> np.ndarray:
    return _vertex_linefit_endpoint(run, frame_index, slot, forward=True, horizon=6)


def _vb6(run, frame_index: int, slot: int) -> np.ndarray:
    return _vertex_linefit_endpoint(run, frame_index, slot, forward=False, horizon=6)
...
def _vb8(run, frame_index: int, slot: int) -> np.ndarray:
    return _vertex_linefit_endpoint(run, frame_index, slot, forward=False, horizon=8)


def _vf10(run, frame_index: int, slot: int) -> np.ndarray:
    return _vertex_linefit_endpoint(run, frame_index, slot, forward=True, horizon=10)


def _vb10(run, frame_index: int, slot: int) -> np.ndarray:
    return _vertex_linefit_endpoint(run, frame_index, slot, forward=False, horizon=10)


def _vf12(run, frame_index: int, slot: int) -> np.ndarray:
    return _vertex_linefit_endpoint(run, frame_index, slot, forward=True, horizon=12)


def _vb12(run, frame_index: int, slot: int) -> np.ndarray:
    return _vertex_linefit_endpoint(run, frame_index, slot, forward=False, horizon=12)
...
def _linear_interval_endpoint(
    ...
    *,
    ...
    numerator = np.sum(
        (temporal_weights * own_weights)[:, None, None]
    ...
    displacement *= np.minimum(1.0, displacement_cap / np.maximum(lengths, 1e-9))[
        :, None
    ]
