...
def _edge_costs(
    ...
    *,
    ...
    crosses = (
        doubled[:, :-1, 0] * doubled[:, 1:, 1] - doubled[:, :-1, 1] * doubled[:, 1:, 0]
    )
    ...
    for distance in range(2, samples + 1):
        ...
        costs[:, distance] = (
            mean + float(tail_weight) * tail + float(worst_weight) * worst
        )
        if distance_weight > 0.0:
    ...
    for anchor in range(samples):
        ...
        for edges in range(1, target + 1):
            ...
            for end in range(lower, upper + 1):
                ...
                values = (
                    previous[starts] + costs[(anchor + starts) % samples, distances]
                )
